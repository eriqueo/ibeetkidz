...
|---|---|---|
| Audio resources (remainder) | Done: melody voices by overlap, bounded derived caches, and voices/players recycled across rebuilds — the measured cause of the reported lag (`BASELINE.md`, field evidence). Left: confirm on Eric's laptop with a second `?perf` report; the first Ride of a session still builds everything once; an audible edit still replaces every lane rather than only the changed one. | A `?perf` report from the affected laptop shows no long task over 100 ms on Ride, tarp or edit. |
