...
|---|---|---|
| Audio resources (remainder) | Done: melody voices by overlap, bounded derived caches, and voices/players recycled across rebuilds. Confirmed on Eric's laptop by the second `?perf` report: tarp on/off cost 24/18 ms frames. The first Ride's build (230–290 ms there) is now rehearsed in small slices when the Track opens (`AudioEngine.rehearse`; 416 → 42 ms of Tone CPU at the press locally). Left: confirm that on Eric's laptop; an audible edit still replaces every lane rather than only the changed one; the Workshop's first Play is not rehearsed. | No long task over 100 ms on the first Ride either. |
| Ride motion | Done: the Track draws from a smoothed audio clock; the raw one jumps a hardware buffer at a time (43 ms on Linux Chrome), which stood the train still on most frames at full frame rate. Confirmed on Eric's laptop: `stillFrames` 0 on every riding second. The clock now takes the frame timestamp, not callback time (that read measured 1.2–3.6× uneven there; ±3% locally now). Left: confirm the evenness in a fourth report; check the same clock on an iPad. | `stillFrames` 0 and `motionUneven` under 1.2 while riding, on the affected laptop. |
| Atlas upload | Done: the cost was the lazy PNG decode inside `texImage2D`, not the copy. Pages are decoded off-thread with `createImageBitmap` (250 → 36 ms a page locally, texels identical); the Map warm-up stays. Left: confirm on Eric's laptop. | No long task over 250 ms on any screen, art unchanged. |
...
| Full-car feedback | Done for My Voice and Voice Keys: a refused lane keeps the panel and the take open and says “This car is full”; an accepted one closes and starts the car loop so the voice is heard. Left: Magic Pad. | Magic Pad retains the take and explains the same way. |
| Tool-panel words and art | Done (interim): a word under every effect and drum icon and under the editor's four controls; effect tiles are a pick, not a stack. Left: the pictures themselves and the unpainted Sound Pads panel (AR-070/071/072); Eric to confirm six effects / six drums; MAKE NOTES plays the raw take, not the chosen effect. | Eric accepts the panels on the live build. |
